...
function getDelay() {
  ...
  try {
    delayArray = JSON.parse(fs.readFileSync('./src/delay.json'));
  } catch (error) {
      ...
      switch (command) {
        ...
        case "pushkontak" : {
          ...
          for (let i = 0; i < get.length; i++) {
            ...
            }, i * getDelay()); // delay setiap pengiriman selama 1 detik
          }
        }
        break;
        case "setdelay" : {
          if (!isCreator) return m.reply(mess.owner)
          if (!text) return m.reply(`Example ${prefix + command} 10000\n\n*_Rekomendasi Menggunakan Kecepatan 10000 (untuk 10 detik)!. Agar Terhindar Dari Suspend Pihak WhatsApp._*`)
          if (text < 10000) return m.reply('Setdelay minimum 10000!')
          if (isNaN(text) || text <= 0) {
            m.reply('```Nilai delay tidak valid atau kurang dari atau sama dengan 0!```');
          } else {
            ...
            // Cari indeks nilai delay dalam array
            const existingIndex = delayArray.findIndex(item => item.delay);
            
            // Jika nilai delay sudah ada, update nilainya
            if (existingIndex !== -1) {
              delayArray[existingIndex].delay = text;
            } else {
              // Jika nilai delay belum ada, tambahkan ke dalam array
